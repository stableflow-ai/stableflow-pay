...
import { getNearConnector } from "@/wallet/near/session";
...
async function walletPublicKey(accountId: string): Promise<string> {
  ...
  if (!connector) throw new Error("Connect a NEAR wallet to register the Intents public key.");
  const wallet = await connector.wallet();
  const accounts = await wallet.getAccounts();
  const match = accounts.find((account: { accountId: string; publicKey?: string }) =>
