/**
 * Cached pay config tokens and chains.
...
import type { PayConfig, PayConfigToken } from "@/types/pay-config";
...
export function mapConfigTokens(raw: PayConfigToken[], chains: ChainConfig[]): IntentsToken[] {
...
interface IntentsTokensState {
  ...
  loading: boolean;
  error: string | null;
  applyConfig: (config: PayConfig | null) => void;
...
export const useIntentsTokensStore = create<IntentsTokensState>()(
  persist(
    ...
    {
      name: "stableflow-pay:intents-tokens:v3-config",
      partialize: (state) => ({
        tokens: state.tokens,
        chains: state.chains,
