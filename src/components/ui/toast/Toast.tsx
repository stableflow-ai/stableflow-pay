    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 font-montserrat text-sm font-semibold leading-normal text-black flex items-start gap-1.5">
          ...
          >
            {isSuccess && <IconCheck className="size-1.5 shrink-0" />}
            {isError && <IconClose className="size-1.5 shrink-0" />}
            {isInfo && <IconAlert className="size-1.5 shrink-0" />}
            {isPending && <IconLoading className="size-2 shrink-0 animate-spin" />}
            {isNotice && <IconAlert className="size-1.5 shrink-0" />}
          </div>
