...
| GET | `/v1/pay/links/{linkId}` | if session | — | `PayPaymentLink` (`organization.logo`) | `getPaymentLink` | `usePaymentLinkQuery` |
...
| `src/stores/guide.ts` | Guide persist (payment link, API key, webhook, `skippedAllUserIds`). Completion is `AuthUser.guideCompleted` from profile / login. |
