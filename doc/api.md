...
- HTTP 401 on an authenticated request calls `useAuthStore.getState().logout()` (`queryClient.clear()` + persist clears `{ token, user }`).
...
| GET | `/v1/pay/links` | yes | — | `PayPaymentLink[]` | `listPaymentLinks` | `usePaymentLinksQuery` |
| POST | `/v1/pay/links` | yes | `PayPaymentLinkBody` | `PayPaymentLink` | `createPaymentLink` | `usePaymentLinkMutations` |
| GET | `/v1/pay/links/{linkId}` | if session | — | `PayPaymentLink` | `getPaymentLink` | `usePaymentLinkQuery` |
...
| POST | `/v1/pay/quote` | if session | `PayQuoteParam` | `PayQuoteResp` | `payQuote` | `usePayQuote` |
| POST | `/v1/pay/swap/link/{linkId}` | if session | `PaySwapParam` | `PaySwapResp` | `paySwapLink` | `usePaySwap` |
| POST | `/v1/pay/swap/checkout/{sessionId}` | if session | `PaySwapParam` | `PaySwapResp` | `paySwapCheckout` | `usePaySwap` |
| POST | `/v1/pay/swap/submit` | if session | `PaySwapSubmitParam` | `string` | `paySwapSubmit` | commit queue |
...
| GET | `/v1/nearintents/status` | if session | `depositAddress` | `NearintentsStatusResp` | `nearintentsStatus` | `useNearintentsStatusQuery` |

Types: `src/types/auth.ts` (`AuthUser`, `LoginBody`, `RegisterBody`, `AuthSession`, `ChangePasswordBody`, `ResetPasswordBody`, `ResetPasswordCodeBody`). Payout types: `src/types/payout.ts`. Analytics: `src/types/analytics.ts`. Recipients: `src/types/recipient.ts`. API keys: `src/types/api-keys.ts`. Partner reports: `src/types/partner.ts`. Organization: `src/types/organization.ts`. Webhooks: `src/types/webhooks.ts`. Payment links: `src/types/payment-links.ts`. Payer quote/swap/checkout: `src/types/pay.ts`. Request Payment: `src/types/request-payment.ts`. Near Intents proxy: `src/types/nearintents.ts`. Settings `/settings` uses organization GET/POST and webhook list / create / delete / enable / disable / rotate-secret. Webhook deliveries, events, get-by-id, and update are wrapped for later UI. Webhook test send has no backend route yet. `GET /v1/nearintents/status` is used by the public waiting page with Bearer only when a session exists. Single and batch quote bodies use 1Click `network` codes (`eth`, `arb`, `sol`, …) plus `token` (`PAYOUT_SYMBOLS`), not 1Click `assetId`. Near-chain `NEAR` still sends symbol `NEAR`; the matching 1Click asset is `nep141:wrap.near` (`wNEAR`, 1:1 with native NEAR). `PaySingleSwapResp` includes `depositAddress` and `orderId` (no `callData`, no `depositMemo`). `PayBatchSwapResp` nests broadcast fields in `transaction` (`approvals` may be `null`, `callData`, `batch_contract` as both payout `to` and ERC-20 spender; Near adds `receiverId` + `actions`; Solana adds `serializedTransaction` + `lastValidBlockHeight`). Single `memo` and `notifyEmail` belong on swap (and are optional on the shared quote type). Request Payment create sends required `name` (max 50) and optional `memo`. List/detail rows also map `payer`, `paid_at`, `destination_tx_hash`, and `withdraw_tx_hash`. The public payer page at `/paylink/:linkId` loads `GET /v1/pay/links/{linkId}` and calls `POST /v1/pay/quote`, `POST /v1/pay/swap/link/{linkId}`, and `POST /v1/pay/swap/submit` with Bearer only when a session token exists. `/checkout?sessionId=` loads `GET /v1/pay/checkout/sessions/{sessionId}` and uses the same quote/submit with `POST /v1/pay/swap/checkout/{sessionId}`. Waiting polls `GET /v1/nearintents/status` the same way. Pending requests can be disabled with `POST /v1/pay/request/{id}/disable`. Batch `receives` use `address` (wallet, no `employeeId`). Payment list rows are snake_case (`submitted_at`, `destination_*`); mappers produce `PayPaymentItem`. Amount/Asset use destination fields. History `start_time` / `end_time` are unix seconds (start of first day, end of last day). History export uses the same filters (no pagination) via `GET /v1/pay/payments/export` (`httpBlob`); the saved filename appends a local `yyyyMMdd-HHmmss` stamp. Volume `period` is `day` / `week` / `month`; points may use `start_at` + `total_payment` instead of `label` / `value`. Origin token pickers for Single / Request Payment are limited by `payerEnabled` on `FIXED_CHAINS`. Batch origin is limited by `batchEnabled` (all registered chains, including native). `/v1/nearintents/*` is our Partner-key proxy of 1Click `/v0`. Withdraw signs via generate-intent then `POST /v1/pay/request/withdraw`. The Pay sidebar polls `GET /v1/pay/request/withdraw/count` every 120s for the Request Payment badge. Authenticated users list and create API keys at `/api-keys` via `/v1/pay/apiKeys` with no Partner registration step. API key labels are max 200 characters. Rename (`POST /v1/pay/apiKeys/{id}`) returns a string; the list uses the submitted `name`. Partner reports stats use `GET /v1/pay/partner/analytics` (`start_time` / `end_time` unix seconds, optional `api_key_id` / `network`). The usage table uses `GET /v1/pay/partner/payments` (paginated; table filters only — no date range). Amount filters map to `min_amount` / `max_amount`. Time column is `submitted_at`. From is `payer`.
...
| `src/api/partner.ts` | Reports analytics and payments |
...
| `src/api/pay.ts` | Payer quote, swap/link, swap/checkout, swap submit |
...
| `src/hooks/use-webhooks-api.ts` | Webhook list query and create / delete / enable / disable / rotate-secret mutations |
| `src/hooks/use-payment-links-api.ts` | Payment link list query and create / delete / enable / disable mutations |
| `src/hooks/use-payment-link.ts` | Public payer payment-link detail query |
| `src/hooks/use-pay-quote-api.ts` | Payer quote query and swap mutation |
| `src/hooks/use-checkout-session.ts` | Public checkout session query |
...
| `src/hooks/use-nearintents-status.ts` | Public payer 1Click status poll (`GET /v1/nearintents/status`) |
...
| `src/types/partner.ts` | Reports analytics and payments types |
| `src/types/organization.ts` | Organization get / update types |
| `src/types/webhooks.ts` | Webhook list, create, deliveries, events, and rotate-secret types |
