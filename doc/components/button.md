...
Native `<button>` with `primary` / `normal` / `danger` variants and size tokens. Disabled and loading states use `opacity: 0.3` and `cursor: not-allowed` (`pointer-events: none`).
...
- Background `#FF5656` (`danger` token), color `#FFF`, same shadow as primary
- Hover: `opacity: 0.9`
...
| --- | --- | --- | --- |
| `variant` | `"primary" \| "normal" \| "danger"` | `"primary"` | |
| `size` | `"xl" \| "lg" \| "md" \| "sm"` | `"md"` | |
