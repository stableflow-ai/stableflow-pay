...
| Function | Notes |
| --- | --- |
| `validateAddress(address, chainKind)` | `{ isValid, error? }` for EVM / Near / Solana / Tron / Zcash. Near uses near-sdk-js / Nomicon account ID rules: length 2–64, lowercase `a-z` / digits / `.` `-` `_`. Zcash accepts transparent t1/t3 and Unified u1 (NEAR Intents; regex, no checksum). Sapling `zs1` is rejected |
...
| `getAddressPlaceholder(chainKind)` | Input placeholder (`t1… / u1…` for Zcash) |
